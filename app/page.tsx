...
export default function Home() {
  ...
  return (
    <div>
            ...
            {menu === "dosen" && filterDosen.map((d) => (
                  <div key={d.id} className="flex items-center justify-between p-2 m-2 text-black bg-white rounded-lg shadow-md w-[calc(100%-1rem)] shrink-0">
                    ...
                    <div className="flex items-center justify-center gap-2">
                      <a href={`mailto:${d.email}`} target="_blank" rel="noopener noreferrer">
                        <div className="flex flex-col items-center justify-center gap-1 border-2 rounded-md p-2">
              ...
              {menu === "callCenter" && filterCallCenter.map((c) => (
                  <div key={c.id} className="flex items-center justify-between p-2 m-2 text-black bg-white rounded-lg shadow-md w-[calc(100%-1rem)] shrink-0">
                    ...
                    <a href={`https://wa.me/+62${c.telp}`} target="_blank" rel="noopener noreferrer">
                      <div className="flex items-center justify-center gap-2 border-2 rounded-md p-2">
      ...
      {/* mobile section */}
      <div className="md:hidden flex flex-col w-screen h-full">
        <div className="fixed w-full z-10">
            <HeaderMobile search={search} setSearch={setSearch} />
            <div className="flex w-full">
              <button onClick={tabDosen} className={`w-full p-2 text-(--text-navbar) bg-(--navbar-menu) hover:bg-(--navbar-menu-hover) ${menu === "dosen" ? "bg-(--navbar-menu-hover)" : ""}`}>Kontak Dosen</button>
              <button onClick={tabCallCenter} className={`p-2 w-full text-(--text-navbar) bg-(--navbar-menu) hover:bg-(--navbar-menu-hover) ${menu === "callCenter" ? "bg-(--navbar-menu-hover)" : ""}`}>Call Center</button>
            </div>
        </div>
        <div className="w-full h-42 bg-blue"></div>
                  ...
                  <div key={c.id} className="flex items-center gap-4 justify-between p-2 m-2 text-black bg-white rounded-lg shadow-md w-[calc(100%-1rem)] shrink-0">
                    <div className="w-full">
                      ...
                      <table>
                        <tbody>
                          <tr>
                            <th className="text-[14px] text-left font-normal">Telepon</th>
                            <th className="text-[14px] text-left font-normal pl-1 pr-1">:</th>
                            <td className="text-[14px]">{c.telp}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
